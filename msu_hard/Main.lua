...
5. CDeaths variable에 Integer를 바로 insert할경우 Trigger가 터짐. 맵이 터질경우 이걸 가장먼저 확인할것

6. SCMD2 트리거 갯수 확인시 약 10만개 이상일시, 지도 미리보기 확인이 불가능하고, 더 추가할경우 트리거가 터질수있음. CSPlot보다 CAPlot을 사용하도록

7. RunAiScript는 CurrentPlayer를 조지는거라 의도치않게 CP값이 복구가 안될때가 있다. RunAiScript 사용 트리거 실행시 모든 트리거가 비활성화 되는 경우는
CopyCpAction등으로 CP를 복구하거나 등의 방법으로 정상화 시켜줘야한다.
